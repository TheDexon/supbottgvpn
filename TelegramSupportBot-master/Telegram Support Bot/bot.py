...
def start(message):
    ...
    bot.send_photo(message.chat.id, photo, caption=f'🌟 Привет! Добро пожаловать в нашу техническую поддержку! Если у вас вопрос или проблема - нажмите "Написать запрос" и ожидайте ответа. Вам придет уведомление. 💻🛠️\n\nЧтобы ответить на сообщение поддержки, выберите "Мои запросы", найдите нужный и нажмите "Добавить сообщение".\n\nПоддержка доступна с 10 до 19 по Московскому времени. Текущее время: {current_time}', parse_mode='html', reply_markup=markup.markup_main())
    photo.close()
